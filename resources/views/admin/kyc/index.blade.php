...
@section('content')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                ...
                <a href="{{ route('admin.partner') }}">KYC</a>
            </li>
        </ul>
    </div>
    ...
    <div>
        <div class="note note-info">
            <p> Admin must view know your customers form detail and approve them </p>
        </div>
        ...
        <div class="portlet light ">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-dark bold uppercase">KYC</span>
                </div>
            </div>
                ...
                <div class="modal-content">
                    <form>
                        ...
                        <div class="modal-footer">
                            <button type="submit" class="btn green"><i class="fa fa-send"></i> <span>Update KYC </span>
                            </button>
                        </div>
                    </form>
        ...
        jQuery(document).ready(function ($) {
            ...
            $grid.init({
                ...
                onSuccess: function (grid, response) {
                    if (pageLoad) {
                        var pageData = window.location.search.substring(1);
                        if (pageData) {
                            var searchData = JSON.parse('{"' + decodeURI(pageData).replace(/"/g, '\\"').replace(/&/g, '","').replace(/=/g, '":"') + '"}');
                            if (typeof searchData.verifykyc !== 'undefined') {
                                setTimeout(function () {
                                    $('[data-action="' + searchData.verifykyc + '"]').find('[data-action="view"]').click();
                                    console.log(searchData.verifykyc);
                                    pageLoad = false;
                                }, 800);
                            }
                        }
            ...
            $datatable.on('click', '.row-actions a[data-action]', function () {
                ...
                var url = '/admin/kyc/edit/' + $e.parents('.row-actions').data('action');

                if ($type === 'view')
                    axios.get(url).then(function (r) {
                        $modal.find('button[type="submit"] span').html('Verify KYC');
                ...
                if ($type === 'delete') {
                    var url = '/admin/kyc/' + $e.parents('.row-actions').data('action');
            ...
            $('#basicModal').on('submit', 'form', function (e) {
                ...
                axios[$e.attr('method')]($e.attr('action'), {active: true}).then(function (r) {
                    new Noty({
                        type: r.data.type,
                        text: '<div><p class="m-0">' + r.data.message + '</p></div>'
                    }).show();
