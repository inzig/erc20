...
@section('content')
    ...
    <div>
        <div class="note note-info">
            <p> Admin must add or update platform </p>
        </div>
        ...
        <div class="portlet light ">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-dark bold uppercase">Platform</span>
                </div>
            </div>
    ...
    <script type="text/javascript">
        jQuery(document).ready(function ($) {
            ...
            $datatable.on('click', '.row-actions a[data-action]', function () {
                ...
                var url = '/admin/platform/edit/'+$e.parents('.row-actions').data('action');
                    if ($type === 'edit')
                    axios.get(url).then(function (r) {
                        $modal.find('button[type="submit"] span').html('Update platform');
            ...
            $('.add-address').click(function () {
                var $modal = $('#basicModal');
                var url = '/admin/platform/create';
                axios.get(url).then(function (r) {
                    $modal.find('button[type="submit"] span').html('Add new platform');
                    $modal.find('.modal-title').html(r.headers.title);
                    $modal.find('.modal-body').html(r.data);
                    $modal.find('form').attr('action', '/admin/platform/add');
