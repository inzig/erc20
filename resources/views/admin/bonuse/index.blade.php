...
@section('content')
    ...
    <div>
        <div class="note note-info">
            <p> Admin must add or update bonuse </p>
        </div>
        ...
        jQuery(document).ready(function ($) {
            ...
            $datatable.on('click', '.row-actions a[data-action]', function () {
                ...
                var $modal = $('#basicModal');
                    ...
                    if ($type === 'edit')
                    axios.get('/admin/bonuse/edit/'+$e.parents('.row-actions').data('action')).then(function (r) {
                        $modal.find('button[type="submit"] span').html('Update bonuse');
            ...
            $('.add-address').click(function () {
                var $modal = $('#basicModal');
                axios.get('/admin/bonuse/create').then(function (r) {
                    $modal.find('button[type="submit"] span').html('Add new bonuse');
                    $modal.find('.modal-title').html(r.headers.title);
                    $modal.find('.modal-body').html(r.data);
                    $modal.modal('show');
                    setTimeout(function () {
            ...
            $('#basicModal').on('submit', 'form', function (e) {
                ...
                axios.patch('/admin/bonuse/add', $e.serialize()).then(function (r) {
                    new Noty({
                        type: r.data.type,
